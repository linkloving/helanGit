package com.VitaBit.VitaBit.prefrences;
...
public static void saveComment(Context context, String strComment){
	...
	//存储更新时间
	public static void setServerUpdateTime(Context context){
		SharedPreferences sharedPreferences;
		try
		{
			sharedPreferences = getAppDefaultSharedPreferences(context, true);
			SharedPreferences.Editor edit = sharedPreferences.edit();
			edit.putLong(KEY_NEED_SHOW_FRAGMENT_TIP,System.currentTimeMillis());
			edit.commit();
		}
		catch (Exception e)
		{
	...
	public static long getServerUpdateTime(Context context){
		SharedPreferences sharedPreferences;
		long jString;
		try {
			sharedPreferences = getAppDefaultSharedPreferences(context, true);
			jString=sharedPreferences.getLong(KEY_NEED_SHOW_FRAGMENT_TIP,0);
			return  jString;
		} catch (Exception e) {
			e.printStackTrace();
		}
