...
public class DetailChartControl extends RelativeLayout {
    ...
    {
        this.imageView1 = (ImageView) findViewById(com.VitaBit.VitaBit.R.id.sleep_chat1);
     ...
     * @return
     */
    private void AsyncAddChart(final List<SportRecord> list)
    {
        new AsyncTask<Void,Void,List<Bitmap>>(){
            @Override
            protected List<Bitmap> doInBackground(Void... params) {
                List<Bitmap> bitmaps=new ArrayList<Bitmap>();
                Bitmap bitmapChart2=detailBitmapCreator2.drawDetailChartStand(list,dayindexNow);
                bitmaps.add(bitmapChart2);
                return bitmaps;
            }
            @Override
    ...
    public void initChart(ArrayList<SportRecord> list,int dayindex)
    {
        imageView1.setImageBitmap(null);
        dayindexNow=dayindex;
        AsyncAddChart(list);
