...
public class DetailBitmapCreator {
	...
	public Bitmap drawDetailChartStand(List<SportRecord> sportRecords,int dayindexNow)
	{
		MyLog.i(TAG,"转换后的数据"+sportRecords.size());
		...
		drawStandChart(sportRecords, canvas);
		drawX(rectPaint, canvas);
		return chartBitmap;
	}


	private void drawStandChart(List<SportRecord> sportRecords,Canvas canvas){
		Paint rectPaint = new Paint();
		...
		for(int i = 0; i< sportRecords.size(); i ++){
			...
			Path path = new Path();
			//此时是站着的
			if(Integer.parseInt(data.getState())==1 || Integer.parseInt(data.getState())==2){
				//画一个矩形
				path.moveTo(getBeginX(data), chartParameter.getChartHeight()-30);
				path.lineTo(getBeginX(data),GetSleepY());
				path.lineTo(getEndX(data),GetSleepY());
...
//		canvas.drawRect(getBeginX(detailData.get(beginIndex)), chartParameter.getChartHeight(), getEndX(detailData.get(endIndex)), chartParameter.getHeight(), rectPaint);
	...
	private float getBeginXScale(SportRecord  data)
	{
			//和现在是同一天的时间,要加时间片加到六个小时的......
			//每个时间片（30s）所占的像素  * 这个数据开始的时间片 + 1/5屏幕
			float width =chartParameter.getXScale()+chartParameter.getWidth() / 5;
			MyLog.i("getBeginX获取到的高度:" +width);
			return width;
	...
	private float GetSleepY(){
		MyLog.i("GetSleepY获取到的高度:" + chartParameter.getChartHeight() + "   " + 60 * chartParameter.getYScale());
		return chartParameter.getChartHeight()-60*chartParameter.getYScale();
	}
