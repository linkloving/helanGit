package com.VitaBit.VitaBit.db.sport;
...
import com.VitaBit.VitaBit.db.basic.TableRoot;
...
//			WidgetUtils.showToast(context, "** 您的操作过于频繁，数据载入失败，请重试！", ToastType.WARN);
        ...
        for (int i = 0; i < cpList.size(); i++) {
            ...
            if (cpList.get(i).getState().equals(Integer.toString(127))) {
                ...
                long timeStart = 0;
                try {
                    Date parse = simpleDateFormat.parse(cpList.get(i).getStart_time());
                    timestate = parse.getTime();
                } catch (ParseException e) {
                    e.printStackTrace();
                }
                do {
                    for (int index = 1; index > -1; index++) {
                        if (cpList.get(i - index).getState().equals(Integer.toString(112))) {
                            cpList.get(i - index).setState("0");
                        } else {
                            break;
                        }
                        try {
                            Date parse = simpleDateFormat.parse(cpList.get(i - index).getStart_time());
                            timeStart = parse.getTime();
                        } catch (ParseException e) {
                            e.printStackTrace();
                        }
                    }
                } while ((timestate - timeStart) < 3600000);
            }
            //如果状态值是123(站)就把之前10分钟内的数据的state改为0
            if (cpList.get(i).getState() .equals(Integer.toString(123)) ) {
                long timestate = 0;
                long timeStart = 0;
                try {
                    Date parse = simpleDateFormat.parse(cpList.get(i).getStart_time());
                ...
                do {
                    for (int index = 1; index > -1; index++) {
                        if (cpList.get(i - index).getState().equals(Integer.toString(113))) {
                            cpList.get(i - index).setState("0");
                        } else {
                            break;
                        }
...
//			WidgetUtils.showToast(context, "** 您的操作过于频繁，数据载入失败，请重试！", ToastType.WARN);
    ...
    public static ArrayList<SportRecord> findHistoryChartHour(Context context, String userId, String startdata, String enddata, boolean is24H) {
        ArrayList<SportRecord> srs = new ArrayList<SportRecord>();
        UserDeviceRecord chatMessageTable = null;
        try {
            // 初始化表操作对象
            ...
            try {
                if (is24H) {
                    ...
                    //这一步 把2016-04-08 转为2016-04-08 15：59：59：999
                    enddata = TimeZoneHelper.__getUTC0FromLocalTime(ToolKits.DATE_FORMAT_YYYY_MM_DD_HH_MM_SS_SSS, enddata + " 23:59:59.999");
                    where = COLUMN_START_TIME + ">='" + startdata + "' and " + COLUMN_START_TIME + "<'" + enddata + "'";
                } else {
                ...
                // 时间范围内的数据
                MyLog.e(TAG, "构建的查询数据库语句是：" + where);

//				select * from DBsportRecord where user_id = '12345' and \
//				((start_time > 1472011200 and start_time < 1472022000) or start_time + duration > 1472011200 and start_time < 1472022000)
