...
import com.linkloving.helan.db.weight.UserWeight;
import com.linkloving.helan.db.weight.WeightTable;
...
import com.linkloving.helan.utils.CountCalUtil;
import com.linkloving.helan.utils.DateSwitcher;
import com.linkloving.helan.utils.ScreenHotAsyncTask;
import com.linkloving.helan.utils.SwitchUnit;
...
//        lineDataSets.add(lineDataSet); // add the datasets
    ...
    private void setDataToView(final int position, final List<UserWeight> myUserWeights){
        ...
        LinearLayout titlelaout = (LinearLayout) view.findViewById(R.id.title_layout);//没数据的时候显示
        //距离目标体重差距
        final TextView myWeight= (TextView) view.findViewById(R.id.step_count);
        ...
        changeGoalWeight.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
               /* //弹出对话框,去更改目标
                final Dialog alertDialog7=new Dialog(WeightActivity.this);
                alertDialog7.setTitle(ToolKits.getStringbyId(WeightActivity.this, R.string.body_info_weight));
                ...
                weight_ok.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (CommonUtils.isStringEmpty(weightEdittext.getText().toString().trim())) {
                            ...
                            weight_goal=Float.parseFloat(weightEdittext.getText().toString());
                            PreferencesToolkits.setGoalInfo(WeightActivity.this, PreferencesToolkits.KEY_GOAL_WEIGHT, weightEdittext.getText().toString());
                            float mfloat=weight_goal-getCount(myUserWeights)/myUserWeights.size();
                            if(mfloat>=0){
                                NumberFormat nf=new DecimalFormat( "0.0 ");
                                myWeight.setVisibility(View.VISIBLE);
                                unit.setVisibility(View.VISIBLE);
                                myWeight.setText(nf.format(mfloat));
                            }else {
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        if (CommonUtils.isStringEmpty(weightEdittext.getText().toString().trim())) {
                                            ...
                                            weight_goal=Float.parseFloat(weightEdittext.getText().toString());
                                            PreferencesToolkits.setGoalInfo(WeightActivity.this, PreferencesToolkits.KEY_GOAL_WEIGHT, weightEdittext.getText().toString());
                                            float mfloat=Math.abs(getCount(myUserWeights)/myUserWeights.size()-weight_goal);
                                            if(mfloat>0){
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        if (CommonUtils.isStringEmpty(weightEdittext.getText().toString().trim())) {
                                            ...
                                            List<UserWeight> weightlist = new ArrayList<>();
                                            weightlist.add(new UserWeight(new SimpleDateFormat(ToolKits.DATE_FORMAT_YYYY_MM_DD).format(new Date()), MyApplication.getInstance(WeightActivity.this).getLocalUserInfoProvider().getUser_id() + "", weightEdittext.getText().toString()));
    ...
    private float getCount(List<UserWeight> list){
        float myWeightCount=0;

        if(list==null||list.size()<=0){
            return myWeightCount;
        }else {

            for(UserWeight userWeight:list){

                myWeightCount=myWeightCount+Float.parseFloat(userWeight.getWeight());
