...
import com.linkloving.helan.ILifecycle;
...
import com.linkloving.helan.basic.CustomProgressBar;
...
import com.linkloving.helan.logic.UI.device.DeviceActivity;
import com.linkloving.helan.logic.UI.device.FirmwareDTO;
import com.linkloving.helan.logic.UI.main.boundwatch.BoundActivity;
import com.linkloving.helan.logic.UI.main.materialmenu.Left_viewVO;
import com.linkloving.helan.logic.UI.main.materialmenu.MenuNewAdapter;
import com.linkloving.helan.logic.UI.main.materialmenu.MenuVO;
import com.linkloving.helan.logic.UI.more.MoreActivity;
import com.linkloving.helan.logic.dto.Profile;
...
import com.linkloving.helan.utils.PermissionUtil;
import com.linkloving.helan.utils.ScreenUtils;
import com.linkloving.helan.utils.ToolKits;
import com.linkloving.helan.utils.logUtils.MyLog;
import com.linkloving.helan.utils.manager.AsyncTaskManger;
...
public class PortalActivity extends AutoLayoutActivity implements MenuNewAdapter.OnRecyclerViewListener{
    ...
    @InjectView(R.id.user_head_layout) CircleClipLayout user_head_layout;
    @InjectView(R.id.user_head) ImageView user_head;//头像
    @InjectView(R.id.portal_name_view) TextView user_head_name; //头像中的文字

    @InjectView(R.id.user_name) TextView user_name;//昵称
//    @InjectView(R.id.logout) LinearLayout logout;//登出

    @InjectView(R.id.device_img) ImageView device_img; //电量图片
    @InjectView(R.id.step_img) ImageView step_img;     //步数图片
    @InjectView(R.id.sit_img) ImageView sit_img;       //坐下
    @InjectView(R.id.stand_img) ImageView stand_img;   //站立

    @InjectView(R.id.text_battery) TextView text_Battery;
    @InjectView(R.id.text_step) TextView text_Step;
    @InjectView(R.id.text_sit) TextView text_Sit;
    @InjectView(R.id.text_stand) TextView text_stand;
    ...
    @InjectView(R.id.progressBar_sit) CustomProgressBar Sit_ProgressBar;
    @InjectView(R.id.progressBar_stand) CustomProgressBar stand_ProgressBar;
    @InjectView(R.id.progressBar_sleep) CustomProgressBar Sleep_ProgressBar;
    /**ITEM布局系列*/
    @InjectView(R.id.linear_date) LinearLayout date;

    @InjectView(R.id.layout_bund) RelativeLayout layout_bund;
    @InjectView(R.id.linear_unbund) LinearLayout linear_unbund;
    @InjectView(R.id.add_device) ImageView add_device;
    @InjectView(R.id.linear_step) LinearLayout linear_Step;
    @InjectView(R.id.linear_sit) LinearLayout linear_Distance;
    @InjectView(R.id.linear_stand) LinearLayout linear_Stand;
    ...
    @InjectView(R.id.nav_headView) LinearLayout nav_headView;
    //修改日期左右的按钮
...
//        initGoal();
    ...
    private void initView() {
        ...
        contentLayout.setBackgroundColor(getResources().getColor(R.color.yellow_title));
...
//                //将数据显示在控件上
    ...
    private void refreshView(double step, double sittime, double standtime) {
        ...
        text_Step.setText(step + getResources().getString(R.string.space) + getResources().getString(R.string.unit_min));
        text_Sit.setText(sittime+ getResources().getString(R.string.space) + getResources().getString(R.string.unit_min));
        //  ext_Cal= (TextView) findViewById(R.id.text_cal);
        text_stand.setText(standtime + getResources().getString(R.string.space) + getResources().getString(R.string.unit_min));
        ...
        if(step_percent>PROGRESS_DEFULT){ //进度超过90%就显示绿色
            step_img.setImageResource(R.mipmap.main_steps_full);
            text_Step.setTextColor(getResources().getColor(R.color.battery_green));
            Step_ProgressBar.setProgressColor(getResources().getColor(R.color.battery_green));
        }else{
            step_img.setImageResource(R.mipmap.main_step);
            text_Step.setTextColor(getResources().getColor(R.color.add_device_blue));
            Step_ProgressBar.setProgressColor(getResources().getColor(R.color.add_device_blue));
        }
        int sit_percent;
        if(sit_goal==0){
            sit_percent = 0;
        ...
        if(sit_percent==100){
            sit_img.setImageResource(R.mipmap.main_sit_full);
            text_Sit.setTextColor(getResources().getColor(R.color.battery_green));
            Sit_ProgressBar.setProgressColor(getResources().getColor(R.color.orangered));
        }
        else if(sit_percent>PROGRESS_DEFULT){
            sit_img.setImageResource(R.mipmap.main_sit_full);
            text_Sit.setTextColor(getResources().getColor(R.color.battery_green));
            Sit_ProgressBar.setProgressColor(getResources().getColor(R.color.battery_green));
        }else{
            sit_img.setImageResource(R.mipmap.main_sit);
            text_Sit.setTextColor(getResources().getColor(R.color.add_device_blue));
            Sit_ProgressBar.setProgressColor(getResources().getColor(R.color.add_device_blue));
        }
        int stand_percent;
        if(stand_goal==0){
            stand_percent = 0;
        ...
        if(stand_percent > PROGRESS_DEFULT){
            stand_img.setImageResource(R.mipmap.main_stand_full);
            text_stand.setTextColor(getResources().getColor(R.color.battery_green));
            stand_ProgressBar.setProgressColor(getResources().getColor(R.color.battery_green));
        }else{
            stand_img.setImageResource(R.mipmap.main_stand);
            text_stand.setTextColor(getResources().getColor(R.color.add_device_blue));
            stand_ProgressBar.setProgressColor(getResources().getColor(R.color.add_device_blue));
        }
        //修改颜色和图片
        refreshBatteryUI();
    }
            ...
            if (provider.isConnectedAndDiscovered()) {  //蓝牙连接上的
                ...
                if (!LocalInfoVO.userId.equals("-1")) {
                    ...
                    if (battery < LOW_BATTERY) {
                        ...
                        dialog_battery.show();
                        device_img.setImageResource(R.mipmap.main_power_low);
                        text_Battery.setTextColor(Color.RED);
                        Battery_ProgressBar.setProgressColor(Color.RED);
                        text_Battery.setText(getString(R.string.portal_main_state_connected));//根据电量显示不同的文字提示
                    }else {
                        int battery_percent = (int) (Math.ceil(battery * 100 * 1.0f / 100));
                        device_img.setImageResource(R.mipmap.main_power_connected);
                        text_Battery.setTextColor(getResources().getColor(R.color.battery_green));
                        Battery_ProgressBar.setProgressColor(getResources().getColor(R.color.battery_green));
                        text_Battery.setText(getString(R.string.portal_main_state_connected));//根据电量显示不同的文字提示
...
//            Battery_ProgressBar.setIndeterminate(true);
    ...
    private void refreshBattery(String msg) {
        ...
        device_img.setImageResource(R.mipmap.main_power_disconnected);
        text_Battery.setTextColor(getResources().getColor(R.color.bg_gray));
        Battery_ProgressBar.setProgressColor(getResources().getColor(R.color.bg_gray));
    }
...
//        }
    ...
    private void setSyncTimeFromLocaVo(){
        ...
        if(localInfoVO.getSyncTime()!=0){
            ...
            mScrollView.getHeaderLayout().setPullLabel(getResources().getString(R.string.portal_device_updatetime)+synctime);
        }else{
            String synctime = "";
            mScrollView.getHeaderLayout().setPullLabel(getResources().getString(R.string.portal_device_updatetime)+synctime);
        }
        ...
        if(serverTime!=0){
            ...
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat(ToolKits.DATE_FORMAT_MM_DD_HH_MM_SS);
            String synctime = simpleDateFormat.format(date);
            mScrollView.getHeaderLayout().setReleaseLabel(getResources().getString(R.string.portal_server_updatetime)+synctime);
        }else{
            String synctime = "";
            mScrollView.getHeaderLayout().setReleaseLabel(getResources().getString(R.string.portal_server_updatetime)+synctime);
        }
