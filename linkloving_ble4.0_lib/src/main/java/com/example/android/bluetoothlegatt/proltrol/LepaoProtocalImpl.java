...
//		flash_head=oadmakeCheckSum(9 ,141*4096,flash_head);            //hzk12地址 
	...
	{
		WatchRequset req = new WatchRequset();
		req.appendByte(seq++).appendByte(LepaoCommand.COMMAND_GET_CARD_STATE).makeCheckSum();
	...
	public boolean updateCardState() throws LPException, BLException
	{
		WatchRequset req = new WatchRequset();
		byte open =1;
		req.appendByte(seq++).appendByte(LepaoCommand.COMMAND_GET_CARD_STATE).appendByte(open).makeCheckSum();
